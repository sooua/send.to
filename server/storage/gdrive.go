...
func NewGDriveStorage(ctx context.Context, clientJSONFilepath string, localConfigPath string, basedir string, chunkSize int, logger *slog.Logger) (*GDrive, error) {
...
func getGDriveClient(ctx context.Context, config *oauth2.Config, localConfigPath string, logger *slog.Logger) *http.Client {
...
func getGDriveTokenFromWeb(ctx context.Context, config *oauth2.Config, logger *slog.Logger) *oauth2.Token {
	...
	if _, err := fmt.Scan(&authCode); err != nil {
		logger.Error("Unable to read authorization code", "error", err)
		os.Exit(1)
	}
...
func saveGDriveToken(path string, token *oauth2.Token, logger *slog.Logger) {
	...
	if err != nil {
		logger.Error("Unable to encode oauth token", "error", err)
		os.Exit(1)
	}
}
