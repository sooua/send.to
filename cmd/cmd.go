...
var globalFlags = []cli.Flag{
	...
	&cli.DurationFlag{
		...
		Usage:   "how long to wait for in-flight requests to finish on SIGINT/SIGTERM",
...
func versionCommand(_ *cli.Context) error {
	fmt.Println(color.YellowString("send.to %s: Easy file sharing from the command line", Version))
	return nil
}

// New is the factory for send.to
func New() *Cmd {
	...
	app.Action = func(c *cli.Context) error {
		...
		if v := c.String("log"); v != "" {
			options = append(options, server.LogFile(v))
		} else {
		...
		if v := c.Duration("shutdown-timeout"); v > 0 {
			options = append(options, server.ShutdownTimeout(v))
		}
